...
async function installAndLaunchWindows(
  ...
  const sentinel = readSentinel(dir);
  if (sentinel?.appPath) {
    const appExe = path.join(sentinel.appPath, 'Agent Deck.exe');
  ...
  console.error('Installing Agent Deck...');
  try {
    ...
    if (fs.existsSync(path.join(defaultDir, 'Agent Deck.exe'))) {
      writeSentinel(dir, {
        type: 'nsis-exe',
        appPath: defaultDir,
      });
      return launchWindowsApp(
        path.join(defaultDir, 'Agent Deck.exe')
      );
    }
    console.error(
      'Installation complete. Please launch Agent Deck from your Start menu.'
    );
    return 0;
  }
  ...
  const appExe = path.join(installDir, 'Agent Deck.exe');
  if (fs.existsSync(appExe)) {
    return launchWindowsApp(appExe);
  }
...
function launchWindowsApp(appExe: string): number {
  console.error('Launching Agent Deck...');
