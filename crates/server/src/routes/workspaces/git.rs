...
async fn resolve_agent_deck_identifier(
...
pub async fn merge_workspace(
    ...
    let vk_id = resolve_agent_deck_identifier(&deployment, workspace.id).await;
    let commit_message = format!("{} (agent-deck {})", workspace_label, vk_id);
