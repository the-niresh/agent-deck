...
When running Agent Deck on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
- Agent Deck runs on a remote server (VPS, cloud instance, etc.)
...
Set a prefix for auto-generated branch names. When you create a new workspace or task, Agent Deck automatically creates a git branch for your changes.
...
Specify where Agent Deck stores workspace data. Workspaces are created in a `.agent-deck-workspaces` subdirectory within this path.
...
Configure how Agent Deck alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when Agent Deck runs in a background tab.
- **Push Notifications** - Receive browser notifications even when Agent Deck isn't in focus. Requires browser permission when first enabled.
...
Enable or disable telemetry data collection to help improve Agent Deck.
