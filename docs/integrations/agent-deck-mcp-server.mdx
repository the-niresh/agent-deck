---
title: "Agent Deck MCP Server"
description: "Configure the Agent Deck MCP server"
---

Agent Deck exposes a local MCP (Model Context Protocol) server, allowing you to manage organisations, projects, issues, workspaces, and repositories from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Agent Deck itself.

<Note>
This page covers connecting **external MCP clients** to Agent Deck's MCP server. For configuring MCP servers **within** Agent Deck for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Agent Deck's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Agent Deck MCP server to any [supported coding agent](/supported-coding-agents) **within** Agent Deck.

1. In Agent Deck Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Agent Deck card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.png" alt="MCP Servers configuration page showing how to add Agent Deck MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "agent_deck": {
      "command": "npx",
      "args": ["-y", "agent-deck@latest", "--mcp"]
    }
  }
}
```

`--mcp` launches the local MCP stdio server. Any additional arguments after `--mcp` are passed through to the `agent-deck-mcp` binary.

## Available MCP Tools

The Agent Deck MCP server provides tools for managing organisations, projects, issues, workspaces, and task execution.
...
3. **Automatic Issue Generation**: Your MCP client will use the Agent Deck MCP server to automatically create structured issues in your project
...
Your MCP client will use the `create_issue` tool to break this down into individual issues and add them to your Agent Deck project automatically.
...
### Internal Coding Agents (Within Agent Deck)

A powerful workflow involves using coding agents within Agent Deck that are also connected to the Agent Deck MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual issues directly in the Agent Deck interface
...
Raycast is a popular MCP client that can connect to Agent Deck's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Agent Deck server" />
</Frame>

Configure the Agent Deck MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Agent Deck MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
