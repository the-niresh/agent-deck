...
description: "Deploy Agent Deck Cloud on any server using Docker Compose"
---

This guide covers deploying Agent Deck Cloud on any Linux server using Docker Compose. This approach works with any cloud provider (AWS, DigitalOcean, Hetzner, etc.) or on-premises server.
...
Clone the Agent Deck repository:

```bash
git clone https://github.com/the-niresh/agent-deck.git
cd agent-deck
```
...
```env .env.remote
# Required secrets
AGENTDECK_REMOTE_JWT_SECRET=<your_generated_jwt_secret>
...
# Email (optional — configure one of SMTP or Loops to enable invitation/notification emails)

# Option A: SMTP (works with any provider — Migadu, Postfix, SendGrid SMTP, etc.)
SMTP_HOST=
SMTP_PORT=587
SMTP_USERNAME=
SMTP_PASSWORD=
...
# Object storage — powers BOTH code-review artifacts and issue attachments.
# Any S3-compatible provider works; Cloudflare R2 is the reference setup.
R2_ACCESS_KEY_ID=
R2_SECRET_ACCESS_KEY=
R2_REVIEW_ENDPOINT=https://<ACCOUNT_ID>.r2.cloudflarestorage.com
R2_REVIEW_BUCKET=
```
...
services:
  ...
  remote-server:
    ...
    environment:
      ...
      AGENTDECK_REMOTE_JWT_SECRET: ${AGENTDECK_REMOTE_JWT_SECRET}
...
2. You should see the Agent Deck Cloud login page
