...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/agent-deck/worktrees` (system temp) |
| **Linux** | `/var/tmp/agent-deck/worktrees` |
| **Windows** | `%TEMP%\agent-deck\worktrees` |

You can configure a custom location in **Settings → General → Workspace Directory**. When set, worktrees are stored in `{your-path}/.agent-deck-workspaces`.
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/agent-deck/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/agent-deck/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\agent-deck\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), Agent Deck will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
Agent Deck automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).
